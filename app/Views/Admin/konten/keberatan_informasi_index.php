    ...
    <div class="card-body py-2 px-3">
        <div class="d-flex flex-wrap gap-3 align-items-center justify-content-between">
            <div class="d-flex flex-wrap gap-2 align-items-center">
                ...
                <a href="<?= base_url('admin/konten/keberatan-informasi' . (($searchQuery ?? '') !== '' ? '?q=' . urlencode($searchQuery) : '')) ?>"
                    class="btn btn-sm rounded-pill <?= ($activeStatus ?? null) === null ? 'btn-primary' : 'btn-outline-secondary' ?>">
                    Semua
                </a>
                <?php foreach ($statuses as $slug => $label) : ?>
                    <a href="<?= base_url('admin/konten/keberatan-informasi?status=' . $slug . (($searchQuery ?? '') !== '' ? '&q=' . urlencode($searchQuery) : '')) ?>"
            ...
            <form action="" method="get" class="d-flex gap-2">
                ...
                <?php if (($searchQuery ?? '') !== ''): ?>
                    <a href="<?= base_url('admin/konten/keberatan-informasi' . (($activeStatus ?? null) !== null ? '?status=' . urlencode($activeStatus) : '')) ?>" 
